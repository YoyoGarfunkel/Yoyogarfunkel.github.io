...
define([
  ...
  function (declare, _WidgetsInTemplateMixin, BaseWidget, TabContainer, List, CountryCodes,
    ...
    SpatialReference, ProjectParameters, webMercatorUtils, WidgetManager, PanelManager, topic
  ) {
...
//        console.info(result.address);
      ...
      _locateAddress: function () {
        ...
        var params = {};
        params.address = {};
        params.outFields = ['Loc_name','Score','Addr_type','X','Y','DisplayX','DisplayY','LongLabel','ExInfo'];
      ...
      addresslocateResult: function (addresses) {
        if (addresses.length > 0){
          this.createLocateResults(addresses).then(lang.hitch(this, function(results){
            this.locateResultArr = results;
            this.divResultMessage.textContent = this.nls.resultsfoundlabel + ' ' + this.locateResultArr.length;
            if (this.locateResultArr.length > 0){
              this.showLocation(this.locateResultArr[0]);
            }
      ...
      createLocateResults: function(addresses) {
        var result = [], pnt;
        var def = new Deferred(), projecting = false;
        array.forEach(addresses, lang.hitch(this,function(addrCandidate, i){
          if(addrCandidate.score >= this.minscore){
            ...
            if(addrCandidate.attributes.DisplayX && addrCandidate.attributes.DisplayY){
              pnt = new Point(addrCandidate.attributes.DisplayX, addrCandidate.attributes.DisplayY, new SpatialReference({wkid:4326}));
            }
            if(this.map.spatialReference.wkid !== this.config.coordinateWKID){
              projecting = true;
              var projParams = new ProjectParameters();
        ...
        if (long && lat){
          ...
          if(selUnit.wkid === this.map.spatialReference.wkid || selUnit.wgs84option == "map"){
            ...
            if(selUnit.tfwkid){
              ...
              projParams.transformForward = selUnit.transformDirection === 'forward' ?  true : false;
            }
